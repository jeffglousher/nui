...
func TestHub_NoGoroutineLeakOnSubscriptionChurn(t *testing.T) {
	...
	// Settle and record the baseline goroutine count.
	runtime.GC()
	...
	// Simulate the websocket disconnect teardown.
	...
	// Relay goroutines must drain back to the baseline once subscriptions are
	// purged. On the leaky implementation the count stays ~base+cycles forever.
